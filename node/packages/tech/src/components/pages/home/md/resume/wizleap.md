...
##### React/Vue両対応の自社UIコンポーネントライブラリのプロダクト運営
...
- Links
  - [社内UIライブラリを作る](https://zenn.dev/wizleap_tech/articles/822f99945ae2b6)（立ち上げ初期に主導したメンバーが執筆した技術記事）
...
##### Microservices化を見越したGo + Clean Architecture + DDD + CQSでの新規開発・一部の既存システムのマイグレート
...
またこちらと並行して、タスクが分担された状態でもバック・フロント間のコミュニケーションが円滑にできるよう、OpenAPIを用いたスキーマ駆動開発を導入し、設計段階での齟齬が発生しにくい体制を同時に整備した。
